...
public:
	...
	JenovaSprite(int w, int h)
	{
		Create(w, h);
	}

	JenovaSprite(std::wstring sFile)
	{
		if (!Load(sFile))
			Create(8, 8);
	}
...
public:
	...
	void DrawSprite(int x, int y, JenovaSprite *sprite)
	{
		if (sprite == nullptr)
			return;
	...
	void DrawPartialSprite(int x, int y, JenovaSprite *sprite, int ox, int oy, int w, int h)
	{
		if (sprite == nullptr)
			return;
...
public:
	void Start()
	{
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&JenovaConsole::GameThread, this);
...
protected: // Audio Engine =====================================================================
	...
	std::vector<JenovaAudioSample> vecAudioSamples;
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&JenovaConsole::AudioThread, this);
	...
	{
		((JenovaConsole*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> JenovaConsole::m_bAtomActive(false);
std::condition_variable JenovaConsole::m_cvGameFinished;
std::mutex JenovaConsole::m_muxGame;
#pragma once
